...
double dPS3_dPT2(double &xa, double &xb, double &M2, double &pt2,
    ...
    if (pt2 < 0) {
        ...
        exit(0);
    } else if (pt2 > std::pow(sh - std::pow(m1 + m2, 2), 2) / sh * .25 && params->out1 < 20) { // not for leptons
    ...
    djac *= kln(M2, m1s, m2s) * std::sin(Tp)
        / std::sqrt(std::pow(s - M2, 2) - 4.0 * s * pt2) / std::pow(4.0 * M_PI, 4);
...
std::complex<double> dPSN_dPT2(std::complex<double> &nm, double &s,
    ...
    const double M2max = sh - 2.0 * std::sqrt(sh * pt2);
...
double IG_dPT2(double *x, size_t dim, void *jj) {
        ...
        for (int i1 = 0; i1 < 5; i1++) {
            ...
            if (is_charge_conserved(params->in1, params->in2, params->out1, params->out2)) {
              if (is_gaugino_gluino(params->out1, params->out2)) {
                real_gluon0 = real_gluon_gaugino_gluino(s, mi2, pt2,
                                                        th, ph, 0, params);
                real_gluon1 = real_gluon_gaugino_gluino(s, mi2, pt2,
                                                        th, ph, 1, params);
    ...
    return dij * std::pow(M_PI, 2) / 4.5 / s * sig * djac * aS(params->murs, params->set) / (2 * M_PI);
    }
}
...
double IQ_dPT2(double *x, size_t dim, void *jj) {
    ...
    // Flux, symmetry factor, spin and color average
    return dij * std::pow(M_PI, 2) / 12.0 / s * sig * djac * aS(params->murs, params->set) / (2 * M_PI);
}
}
