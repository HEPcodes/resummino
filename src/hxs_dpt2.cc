...
double dPS3_dPT2(double &xa, double &xb, double &M2, double &pt2, double &Tp,
  ...
  djac *= kln(M2, m1s, m2s) * std::sin(Tp) /
          std::sqrt(std::pow(s - M2, 2) - 4.0 * s * pt2) /
          std::pow(4.0 * M_PI, 4);

  return djac;
}

std::complex<double> dPSN_dPT2(std::complex<double> &nm, double &s, double &t,
...
double IG_dPT2(double *x, size_t dim, void *jj) {
  ...
  if (is_gaugino_gluino(params->out1, params->out2)) {
    ...
    return dij * std::pow(M_PI, 2) / 4.5 / s * sig * djac *
...
double IQ_dPT2(double *x, size_t dim, void *jj) {
  ...
  if (is_gaugino_gluino(params->out1, params->out2)) {
    ...
    // Flux, symmetry factor, spin and color average
    return dij * std::pow(M_PI, 2) / 12.0 / s * sig * djac *
           aS(params->murs, params->set) / (2 * M_PI);
  }
}
...
double IR_dPT2(double *x, size_t dim, void *prm) {
  ...
  std::complex<double> hbssl =
      -M_1_PI * std::exp(-II * b * pt * std::sin(theta));
...
void hadronic_xs_dPT2(double &res, double &err, double &chi2, int Flag,
  ...
  case 4:
    ...
    break;
  case 5:
    Integration(&IR_dPT2, 5, 60000, params->precision, 1e-12, res, err, params,
                0.2, 5, 5);
    break;
  case 6:
    Integration(&IJ_dPT2, 5, 60000, params->precision, 1e-12, res, err, params,
                0.2, 5, 5);
    break;
