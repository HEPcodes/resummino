...
double real_gluon_gaugino_gluino(const double S, const double M2, const double PT2,
...
// Partonic cross section for real quark emission without on-shell contributions.
double real_quark_gaugino_gluino(const double S, const double M2, const double PT2,
                                 const double TH, const double PH, const int YS, Parameters *params) {
    double born = 0.0;
...
// Partonic cross section for real antiquark emission without on-shell contributions.
...
// Partonic cross section for real quark emission where s23 can lead to an on-shell resonance.
double real_quark_gaugino_gluino_onshell_23(const double S, const double S2, const double T1,
                                            const double S1, const double PHI, 
                                            int flag, Parameters *params) {
    double born = 0.0;
            ...
            if ((2.0 * ff->papb >= pow2(params->mSQ[i0] + ff->m1) 
              ...
              ff->SetPropagator(params->mSQ[i0],
                ...
                if (i0 == i1 && 2.0 * ff->papb >= pow2(params->mSQ[i0] + ff->m1) 
                    ...
                    ff->SetBreitWigner(2.0 * ff->p2p3 + ff->m2s,pow2(params->mSQ[i0]),params->mSQ[i0] * WIDTH);
                    // On-shell kinematics.
...
// Partonic cross section for real quark emission where s13 can lead to an on-shell resonance.
double real_quark_gaugino_gluino_onshell_13(const double S, const double S2, const double T1,
                                            const double S1, const double PHI, int flag, Parameters *params) {
    double born = 0.0;
    double factor = 0.0;
...
// Partonic cross section for real antiquark emission where s13 can lead to an on-shell resonance.
...
// Partonic cross section for real antiquark emission where s23 can lead to an on-shell resonance.
...
double Dip_GLGA(DipoleType Emitter_Spectator, PartonType emitter,
                ...
                const double S, const double M2, const double PT2, const double TH,
    ...
    case INITIAL_INITIAL:
        if ( emitter == PARTON_QUARK && spectator == PARTON_ANTIQUARK
            ...
            // Dipole factor.
            Dipole =  D_ai_b(PARTON_QUARK, PARTON_GLUON, ff->pap3, ff->papb,
                             ff->pbp3, x, 1, 0.5 * (cA - 2.0 * cF));
            ...
            Dipole =  D_ai_b(PARTON_GLUON, PARTON_ANTIQUARK,ff->pbp3, ff->papb,
                             ff->pap3, x, 1,   0.5 * (cA - 2.0 * cF) /cF * TR);

        } else if ( emitter == PARTON_GLUON && spectator == PARTON_ANTIQUARK
                    && emitted_parton == PARTON_ANTIQUARK ) {// D_gq_q

            ff->SetDipKinematicAB(x);
            Dipole =  D_ai_b(PARTON_GLUON, PARTON_QUARK,ff->pap3, ff->papb,
                             ff->pbp3, x, 1,  0.5 * (cA - 2.0 * cF) / cF * TR);
    ...
    case INITIAL_FINAL:
        if ( emitter == PARTON_QUARK && spectator == PARTON_GLUINO
            ...
            Dipole = D_ai_j(PARTON_QUARK, PARTON_GLUON, zi, zj, ff->pap3,
                            ff->p1p3,  x,  1 , - 0.5 * cA );
            ...
            Dipole = D_ai_j(PARTON_ANTIQUARK, PARTON_GLUON, zi, zj, ff->pbp3,
                            ff->p1p3,  x,  1 , - 0.5 * cA);
            ...
            ff->SetDipKinematicA1(x, zi, zj);
            Dipole = D_ai_j(PARTON_GLUON, PARTON_ANTIQUARK, zi, zj,
                            ff->pap3, ff->p1p3,  x,  1 , - 0.5 * cA/cF * TR);

        } else if ( emitter == PARTON_GLUON && spectator == PARTON_GLUINO
                    && emitted_parton == PARTON_QUARK ) { // D_gq_gl

            ff->SetDipKinematicB1(x, zi, zj);
            Dipole = D_ai_j(PARTON_GLUON, PARTON_QUARK, zi, zj, ff->pbp3,
                            ff->p1p3,  x,  1 , - 0.5 * cA/cF * TR);

        }
    case FINAL_INITIAL:
        if ( emitter == PARTON_GLUINO && spectator == PARTON_QUARK
             && emitted_parton == PARTON_GLUON ) {

            ff->SetDipKinematic1A(x, zi, zj, zplus, zminus, ff->m1, 0.0, ff->m1);
            Dipole =  D_ij_a(PARTON_GLUON, PARTON_GLUINO, 0.0, params->mGL,
                             params->mGL, ff->p1p3,  x,  1 , zj,
                             params->mGL, zi, zplus, zminus, -0.5 * cA);

        } else if ( emitter == PARTON_GLUINO && spectator == PARTON_ANTIQUARK
                    && emitted_parton == PARTON_GLUON ) {
            ff->SetDipKinematic1B(x, zi, zj, zplus, zminus, ff->m1, 0.0, ff->m1);
            Dipole =  D_ij_a(PARTON_GLUON, PARTON_GLUINO, 0.0,
                             params->mGL, params->mGL, ff->p1p3,  x,  1 , zj,
                             params->mGL, zi, zplus, zminus, -0.5 * cA);
        }
    }
