...
// Flavor constants given in hep-ph/0201036 p.53 eq. C.12 and p.32 eq. 5.91 and p.37 eq. 6.17
...
// General expression for the Altarelli Parisi splitting function (hep-ph/0201036 p. 32, eq. 5.94).
double P_aap(PartonType a, PartonType ap, FunctionType function_type, double x, double z) {
  ...
  // Ta * Ta for a == quark or a == gluon (Expectation values of the color operators)
  double color;
...
// Epsilon dependent parts of d-dimensional AP splitting functions (hep-ph/0201036 p. 32 eq 5.93).
double Php_aap(PartonType a, PartonType ap, double x) {
  if ( (a == PARTON_QUARK && ap == PARTON_QUARK) || (a == PARTON_ANTIQUARK && ap == PARTON_ANTIQUARK)) {
    return cF * (1 - x);
...
double K_aap_bar(PartonType a, PartonType ap, FunctionType function_type, double x, double z) {
  ...
  case FUNCTION_DELTA:
    ...
    // (solved with mathematica and a substitution 1-x -> y to "help" mathematica)
    return  delta_aap * (- color/3.0 * (pow(M_PI,2) - 3.*pow(log(1 - z),2) - 6.*gsl_sf_dilog(1 - z)) 
...
double J_a_ij(PartonType i, PartonType j, double muj, FunctionType function_type, double x, double z) {
    //J_gQ (J_gQb)
    if (i == PARTON_GLUON && ( j == PARTON_QUARK || j == PARTON_ANTIQUARK || j == PARTON_GLUINO)) {
        ...
        case FUNCTION_DELTA: // again substitution used in mathematica 1-x -> y to obtain a "nice" result      
            return -(-((pow(muj,2)*z)/((1 + pow(muj,2))*(1 + pow(muj,2) - z))) 
                     ...
                     - 4.*gsl_sf_dilog((-1 + z)/pow(muj,2)))/2.;      
        case FUNCTION_PLUS:
            return (1.0 - x)/(2.0*pow2((1.0 - x + pow2(muj)))) 
                - 2.0 / (1.0 - x) * (1.0 + log(1.0 - x + pow2(muj) ) ) 
                + 2.0/(1.0 - x) * log(2.0 + pow2(muj) - 1.0); 
            // the last log is not part of the plus distribution, therefore there is a 1 instead of an x
        }
    }
}
...
double Kappa_aap_j(PartonType a, PartonType ap, PartonType j, 
                   double mj, double sja, FunctionType function_type, double x, double z) {
  ...
  if (a == PARTON_GLUON && ap == PARTON_QUARK 
    ...
    case FUNCTION_PLUS:
      return 2.0 * (log(1-x)/(1-x)) + J_a_ij(PARTON_GLUON,j, mj/sqrt(sja),FUNCTION_PLUS,x,z)
    ...
    case FUNCTION_PLUS:
      return 0.0;
             ...
             && (j == PARTON_QUARK || j == PARTON_GLUINO)){  //Kappa_gg_q or Kappa_gg_gl
    switch(function_type) {
    case FUNCTION_ALL_WDELTA:
      return 2.0 * pow2(mj)/(x * sja) * log(pow2(mj)/((1-x)*sja) + pow2(mj)) // Kappa_qg_q
        + 2.0 * (log(1-x)/(1-x) - log(2-x)/(1-x)) 
        + J_a_ij(PARTON_GLUON,PARTON_QUARK, mj/sqrt(sja),FUNCTION_ALL_WDELTA,x,z) 
        + 2.0 * 1/(1-x)*log((2-x)*sja/ ( (2-x)*sja + pow2(mj) ) ); // Kappa_gg_q
    case FUNCTION_DELTA:
      return (log(1 - z)*(2*log(sja/(pow(mj,2) + sja)) + log(1 - z))) - GAMMA_Q/cF
        + pow2(mj)/sja * log(pow2(mj)/(sja + pow2(mj)) ) + 0.5 * pow2(mj)/(sja + pow2(mj))
        + J_a_ij(PARTON_GLUON,PARTON_QUARK, mj/sqrt(sja),FUNCTION_DELTA,x,z);
    case FUNCTION_PLUS:
      return 2.0 * (log(1-x)/(1-x)) 
        + J_a_ij(PARTON_GLUON,PARTON_QUARK, mj/sqrt(sja),FUNCTION_PLUS,x,z)
        + 2.0 * 1/(1-x)*log(sja/ (sja + pow2(mj) ) );
    }
  } 
}
...
double P_bold_aap_b_j(PartonType a, PartonType ap, PartonType b,
                      PartonType j, FunctionType function_type, double x, 
                      double z, double sja, double sab, double mufs, double alphas, 
  ...
  if (emission == INITIAL_AND_FINAL) {
    switch(function_type) {
    case FUNCTION_ALL_WDELTA:
      return  alphas/(2*M_PI) * P_aap(a,ap,function_type,x,z) * color1 * log(mufs/(x*sja)) 
        + alphas/(2*M_PI) * P_aap(a,ap,function_type,x,z) * color2 * log(mufs/(x*sab));
    case FUNCTION_DELTA:
    ...
    switch(function_type) {
    case FUNCTION_ALL_WDELTA:
        return  color2 * alphas/(2*M_PI) * P_aap(a,ap,function_type,x,z) * log(mufs/(x*sab));
    case FUNCTION_DELTA:
...
// p.44 and 46
// eq 6.55 + 6.68
double K_bold_aap_b_j(PartonType a, PartonType ap, PartonType b,
                      PartonType j, FunctionType function_type, 
                      double x, double z, double mj, double sja, double sab, 
                      double alphas,EmissionType emission ) {
  ...
  if (emission == INITIAL_AND_FINAL) {
    ...
    case FUNCTION_ALL_WDELTA:
      return alphas/(2*M_PI) * ( K_aap_bar(a,ap,function_type,x,z) 
                                 ...
                                 - color_factor1 * ( P_aap_reg(a,ap,x)
                                                     * log( (1-x)*sja/( (1-x)*sja + pow2(mj) ) ) ))
        -alphas/(2*M_PI) * (color_factor3 * P_aap_reg(a,ap,x) * log(1-x) 
                            + color_factor4 * delta_aap * 2.0 * log(1-x)/(1-x)); 
    case FUNCTION_DELTA:
      return -alphas/(2*M_PI)* color_factor1 * delta_aap 
        * gamma_a * ( log( (sja - 2*mj*sqrt(sja + pow2(mj)) 
                            + 2*pow2(mj) )/ sja) + 2*mj/(sqrt( sja + pow2(mj) ) +mj) )
  ...
  if (emission == INITIAL) {
    ...
    case FUNCTION_ALL_WDELTA:
      return alphas/(2*M_PI) * ( K_aap_bar(a,ap,function_type,x,z))
        -alphas/(2*M_PI) * (color_factor3 *  P_aap_reg(a,ap,x) * log(1-x) + color_factor4 * delta_aap * 2.0 * log(1-x)/(1-x)); 
    case FUNCTION_DELTA:
        return alphas/(2*M_PI) * (-color_factor4) * delta_aap * (pow(log(1 - z),2)  - pow2(M_PI)/3) +  alphas/(2*M_PI) 
...
// this is for a gluon going into the hard process (not needed yet; check and fix)
double V_qq_b(double x, double alpha, double papi, double papb, double pbpi) {
  return 8.0 * M_PI * alpha * ( - 4.0 * x + (1-x)/x * (2.0 * papb / (papi * pbpi) ) ); //cF
}


double V_gg_b(double x, double alpha, double papi, double papb, double pbpi) {
  return 16.0 * alpha * ( -4.0 * ( x / (1.0-x) + x * (1.0-x)  ) 
                          + (1.0-x) / x * papb / (papi*pbpi) * (-2.0) * papi / papb * pbpi  ); // cA
}


// Splitting functions V for initial state emitter ai and final state spectator j (V_ai_j)
...
double V_qq_j(double x, double alpha, double zi, double zj, double pjpi) {
  return  8 * M_PI * alpha  * ( - 4.0 * x + (1.0 - x)/x 
                                * 2.0 * zi * zj / pjpi  * ( - 2.0 * pjpi/( zi * zj)) ); // cF
}


// Splitting functions V for final state emitter and initial state spectator (V_a_ij)
...
double D_ai_b(PartonType a, PartonType i, double papi,
              double papb, double pbpi, double x, double alpha, double color) {
  if ((a == PARTON_QUARK || a == PARTON_ANTIQUARK) && i == PARTON_GLUON ) {
    ...
    return - 1.0/(2.0 * papi) * 1.0/x * color * V_gq_b(x,alpha);
      ...
      return - 1.0/(2.0 * papi) * 1.0/x * color * V_qq_b(x,alpha, papi, papb, pbpi);
  }
}

// initial emitter and final spectator
// p.29 eq.5.71
double D_ai_j(PartonType a, PartonType i, double zi, double zj,
              double papi, double pjpi, double x, double alpha, double color) {
  if ((a == PARTON_QUARK || a == PARTON_ANTIQUARK) && i == PARTON_GLUON ) {
    ...
    return - 1.0/(2.0 * papi) * 1.0/x * color * V_qq_j(x, alpha, zi, zj,pjpi) ;
  }
}

// Final emitter and initial spectator
// p.25 eq 5.40
double D_ij_a(PartonType i, PartonType j, double mi, 
              double mj, double mij, double pjpi, double x, double alpha, 
              double zj, double mQ, double zi, double zplus, double zminus , double color) {
  if (i == PARTON_GLUON && (j == PARTON_QUARK || j == PARTON_GLUINO) ) {
    return -1.0/(pow2(mi) + pow2(mj) + 2.0 * pjpi - pow2(mij) ) 
      * 1.0/x * color * V_a_gQ(x, alpha, zj, mQ, pjpi);
  } else if (i == PARTON_GLUON && j == PARTON_QUARK ) {
    // not needed now and maybe wrong (check!) (needed for squark gaugino production)
    return -1.0/(pow2(mi) + pow2(mj) + 2 * pjpi - pow2(mij) ) 
      * 1.0/x * color * V_a_QQB( x, alpha, zplus, zminus, zi);
  }
}
