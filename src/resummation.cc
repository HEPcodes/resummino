...
complex<double> ThG_nll(complex<double> LL, double S, double DIJ, double Muf2, double Mur2, int set, Parameters *params) {
...
//   static const double zeta3 = gsl_sf_zeta_int(3); // (1.20206)
  ...
  const complex<double> g1 = A1 / (2.0 * M_PI * b0 * lmbd) * (2.0 * lmbd + (1.0 - 2.0 * lmbd) * lnlmbd);
  // NLL Sudakov
  complex<double> g2 = 0;
    ...
    for (int i1 = 0; i1 < 5; i1++) {
      ...
      if (is_charge_conserved(params->in1, params->in2, params->out1, params->out2)) {
        ...
        if (is_gaugino_gluino(params->out1, params->out2)) {
          ...
          double P_T = -3.0 / 4.0 * ((2.0 * cF - cA) * log(mufs / S) +
                                     cA * log(mufs / (mgls - T)));
          double P_U = -3.0 / 4.0 * ((2.0 * cF - cA) * log(mufs / S) +
                                     cA * log(mufs / (mgls - U)));

          double K_T =
              pow2(M_PI) / 2.0 * cF - GAMMA_Q - K_Q +
              cA / 4.0 * (1. + 4.0 * gsl_sf_dilog((2.0 * mgls - T) / (mgls)) +
                          (1.0 + 4.0 * log(mgls / (mgls - T)) +
                           2.0 * (mgls / (mgls - T))) *
                              log((mgls / (2.0 * mgls - T))) +
                          3.0 * log(1.0 +
                                    2.0 * mgl / (mgls - T) *
          ...
          double K_U =
              pow2(M_PI) / 2.0 * cF - GAMMA_Q - K_Q +
              cA / 4.0 * (1. + 4.0 * gsl_sf_dilog((2.0 * mgls - U) / (mgls)) +
                          (1.0 + 4.0 * log(mgls / (mgls - U)) +
                           2.0 * (mgls / (mgls - U))) *
                              log((mgls / (2.0 * mgls - U))) +
                          3.0 * log(1.0 +
                                    2.0 * mgl / (mgls - U) *
...
// Resummed XS and expansion up to NLO to avoid double counting.
#ifndef EXPANSION
        RES += (qN[0][i0] * qN[1 - params->ic][i1] +
                qN[params->ic][i0] * qN[1][i1]) *
               (H0 + alphaS / (2.0 * M_PI) * H1);
#endif
        if (is_gaugino_gluino(params->out1, params->out2)) {
          EXP += (qN[0][i0] * qN[1 - params->ic][i1] +
                  qN[params->ic][i0] * qN[1][i1]) *
                 (alphaS / (2.0 * M_PI) * H1 +
                  H0 * (1.0 +
                        alphaS / (M_PI)*lnNb *
                            (-DIJ +
                             2.0 * cF * (log(mufs / params->murs) + lnNb -
                                         log(S / params->murs)))));
        } else {
          EXP +=
              (qN[0][i0] * qN[1 - params->ic][i1] +
               qN[params->ic][i0] * qN[1][i1]) *
  ...
  return (RES * ThG_nll(lnNb, S, DIJ, params->mufs, params->murs, params->set, params) - EXP) * dij / 72.0 / S;
}


// G-exponent function for the resummation with g3 coefficient valid for Drell-Yan like processes with no colour flow (i.e. (s)lepton and electroweakinos pair production)
complex<double> ThG_nnll(complex<double> LL, double S, double Muf2, double Mur2, int set, Parameters *params) {
  ...
  const double b0 = (11.0 * cA - 2.0 * dnf) / (12.0 * M_PI);
  const double b1 = (17.0 * pow2(cA) - 5.0 * cA * dnf - 3.0 * cF * dnf) / (24.0 * pow2(M_PI));
  const double b2 = 1.0 / (64.0 * pow(M_PI, 3)) * (2857.0 / 54.0 * pow(cA, 3)
                    - 1415.0 / 54.0 * pow2(cA) * dnf -
                    205.0 / 18.0 * cA * cF * dnf + pow2(cF) * dnf +
                    pow2(dnf) / 9.0 * (79.0 / 6.0 * cA + 11.0 * cF));
  ...
  const double A1 = 2.0 * cF;
  const double A2 = 2.0 * cF * ((67.0 / 18.0 - zeta2) * cA - 5.0 / 9.0 * dnf);
  const double A3 = 2.0 * 0.25 * cF * (pow2(cA) * (245.0 / 24.0 - 67.0 / 9.0 * zeta2 + 11.0 / 6.0 * zeta3 + 11.0 / 5.0 * pow2(zeta2)) +
                    cF * dnf * (-55.0 / 24.0 + 2.0 * zeta3) + cA * dnf * (-209.0 / 108.0 + 10.0 / 9.0 * zeta2 - 7.0 / 3.0 * zeta3) - pow2(dnf) / 27.0);

  // Only initial state dependent.
  // Needed for NNLL.
  const double D2_i = 2.0 * cF * (cA * (-101.0 / 27.0 + 11.0 / 3.0 * zeta2 + 7.0 / 2.0 * zeta3) + dnf * (14.0 / 27.0 - 2.0 / 3.0 * zeta2));
  ...
  if (!is_gaugino_gluino(params->out1, params->out2)) {
    ...
    g3 = A1 * pow2(b1) / (2.0 * M_PI * pow(b0, 4)) * 1.0 / (1.0 - 2.0 * lmbd) * (2.0 * pow2(lmbd) + 2.0 * lmbd * lnlmbd + 0.5 * pow2(lnlmbd)) +
         (A1 * b2) / (2.0 * M_PI * pow(b0, 3)) * (2.0 * lmbd + lnlmbd + 2.0 * pow2(lmbd) / (1.0 - 2.0 * lmbd)) +
         2.0 * A1 / M_PI * zeta2 * lmbd / (1.0 - 2.0 * lmbd) -
         A2 * b1 / (pow2(2.0 * M_PI) * pow(b0, 3)) * 1.0 / (1.0 - 2.0 * lmbd) * (2.0 * lmbd * (lmbd + 1.0) + lnlmbd) +
         A3 / (pow(M_PI, 3) * pow2(b0)) * pow2(lmbd) / (1.0 - 2.0 * lmbd) -
         D2_i / (2.0 * pow2(M_PI) * b0) * lmbd / (1.0 - 2.0 * lmbd) +
         (A1 * b1 / (2.0 * M_PI * pow2(b0)) * (2.0 * lmbd + lnlmbd) / (1.0 - 2.0 * lmbd)) * log(S / Mur2) +
         A1 / (2.0 * M_PI) * (lmbd / (1.0 - 2.0 * lmbd) * pow2(log(S / Mur2)) - lmbd * pow2(log(Muf2 / Mur2))) -
         A2 / (2.0 * pow2(M_PI) * b0) * (lmbd / (1.0 - 2.0 * lmbd) * log(S / Mur2) - lmbd * log(Muf2 / Mur2));
  }

//   const double alphaS = aS(params->murs, params->set);

    return exp(LL * g1 + g2 + aS(Mur2, set) * g3);
}

// Threshold resummation: (unintegrated) hadronic XS for ordinary NNLL resummation (matching at NLO) valid for Drell-Yan like processes with no colour flow (i.e. (s)lepton and electroweakinos pair production)
complex<double> Thadronic_nnll_xs(complex<double> NM, double S, double T,
                             Parameters *params) {
  ...
  const double alphaS = aS(murs, params->set);
  ...
  // Collinear remainder part of the hard matching coefficient for Drell-Yan
  // like processes.
  double delta_Pqq1 = -3.0 / 2.0 * log(mufs / S);
  ...
  for (int i0 = 0; i0 < 5; i0++) {
    for (int i1 = 0; i1 < 5; i1++) {
      ...
      if (is_charge_conserved(params->in1, params->in2, params->out1, params->out2)) {
        ...
        // NLO and NNLO hard matching coefficients.
        C1 = cF * (4.0 / 3.0 * pow2(M_PI) - 8.0 - 3.0 * log(mufs / S));
        C2 = cF / 720 * ( 5.0 * (-4605.0 * cA + 4599.0 * cF + 762.0 * nf) + 20.0 * pow2(M_PI) * (188.0 * cA - 297.0 * cF - 32.0 * nf) - 92.0 * pow(M_PI,4) * (cA - 6.0 * cF) + 180.0 * (11.0 * cA + 18.0 * cF - 2.0 * nf) * pow2(log(mufs / S)) - 160.0 * (11.0 * cA - 2.0 * nf) * (6.0 - pow2(M_PI)) * log(murs / S) + 80.0 * (151.0 * cA - 135.0 * cF + 2.0 * nf) * zeta3 + 20.0 * log(mufs / S) * (-51.0 * cA + 837.0 * cF + 6.0 * nf - 4.0 * pow2(M_PI) * (11.0 * cA + 27.0 * cF - 2.0 * nf) + (-198.0 * cA + 36.0 * nf) * log(murs / S) + 216.0 * (cA - 2.0 * cF) * zeta3 ) );
...
// Resummed XS and expansion up to NLO to avoid double counting.
  ...
  // Hard matching function times exponential; expansion subctracted; color and
  // spin average; flux factor.
  return (RES * ThG_nnll(lnNb, S, mufs, murs, params->set, params) - EXP) * dij / 72.0 / S;
}
...
complex<double> ThG2(const complex<double> LL, double MIS, double MURS, const int set, Parameters *params) {
  ...
  const double A2 = 2.0 * cF * ((67.0 / 18.0 - pow(M_PI, 2) / 6.0) * cA - 5.0 / 9.0 * dnf);
  ...
  // Exactly Eq. 3.52
  const complex<double> g2 = (2.0 * lmbd + lnlmbd) * (A1 / beta0 * (log(MIS / MURS) + beta1 / pow(beta0, 2)) - A2 / pow2(beta0) + B1 / beta0) -
...
complex<double> Thadronic_xs2(complex<double> N, double S, double T,
                              Parameters *params) {
  // logarithm
  ...
  const complex<double> Pqq = 4.0 / 3.0 * (1.5 + 1.0 / N / (N + 1.0) - 2.0 * (Psi(N + 1.0) + M_EULER));
...
// Hard matching coeff. and evolved PDFs.
#ifndef EXPANSION
        RES += (qE[0][i0] * qE[1 - params->ic][i1] +
                qE[params->ic][i0] * qE[1][i1]) *
        ...
        EXP += (qN[0][i0] + qN[1 - params->ic][i1] + qN[params->ic][i0] +
                qN[1][i1]) *
               gN * (log(S / params->mufs) - 2.0 * ll) * Pqg * alphaS /
               (2.0 * M_PI) * brn;
      }
    }
  }
...
void CFact(complex<double> &g, complex<double> q[2][6], complex<double> N,
           double aSmu, complex<double> aSbb) {
...
complex<double> PtG(const complex<double> LL, double MIS, double MURS,
                    const int set) {
  // Coeff. of the beta function
  const int nf = NFLVR;
  const double dnf = (double)nf;
  ...
  const double A2 =
      .5 * A1 * (67.0 / 3.0 - pow(M_PI, 2) - 10.0 / 9.0 * dnf) / beta0;
  ...
  const complex<double> g2 =
      (lmbd / (1.0 - lmbd) + lnlmbd) *
          (A1 * (log(MIS / MURS) + beta1 / pow(beta0, 2) * (1.0 + lnlmbd)) -
           A2 + B1) -
...
complex<double> PtXS(complex<double> N, complex<double> B, double S, double T,
                     Parameters *params) {
  // logarithm
  ...
  const complex<double> lmbd =
      aa * beta0 * log(params->mufs / S * (1.0 + bb * bb));
  pdfEvolve(gE, qE, N, lmbd);
  ...
  for (int i0 = 0; i0 < 5; i0++) {
    for (int i1 = 0; i1 < 5; i1++) {
      ...
      if (is_charge_conserved(params->in1, params->in2, params->out1,
                              params->out2)) {
        if (is_gaugino_gluino(params->out1, params->out2)) {
          cout << "No pt resummation for associated production." << endl;
          exit(1);
        } else if (is_slepton_slepton(params->out1, params->out2)) {
          brn = born_sleptons(S, T, params);
          vir = Virt_sleptons(S, T, params) + DipI_sleptons(S, T, params);
          vir -= (10.0 - pow2(M_PI)) *
                 brn; // to restore old version with old integrated dipole
        } else if (is_gaugino_gaugino(params->out1, params->out2)) {
          brn = born_gauginos(S, T, params);
          vir = Virt_gauginos(S, T, params) + DipI_gauginos(S, T, params);
          vir -= (10.0 - pow2(M_PI)) *
        ...
        EXP += (qN[0][i0] * qN[1 - params->ic][i1] +
                qN[params->ic][i0] * qN[1][i1]) *
               ll * (-ll * 4.0 / 3.0 + 4.0 - 2.0 * Pqq) * aa * brn;
        EXP += (qN[0][i0] + qN[1 - params->ic][i1] + qN[params->ic][i0] +
                qN[1][i1]) *
               gN * (-ll) * Pqg * aa * brn;
      }
...
// Joint resummation
complex<double> JtG(const complex<double> LL, const complex<double> LN,
  ...
  const complex<double> g1 = A1 * (1.0 + lnlmbd / lmbd);
  const complex<double> g2 =
      (lmbd / (1.0 - lmbd) * (1.0 - lmbdN) + lnlmbd) *
          (A1 * log(MIS / MURS) - A2) +
      A1 * beta1 * (.5 * pow(lnlmbd, 2) +
                    (1.0 - lmbdN) / (1.0 - lmbd) * (lmbd + lnlmbd)) +
      B1 * lnlmbd;

  return exp(LL * g1 + g2);
}

complex<double> JtXS(complex<double> N, complex<double> B, double S, double T,
                     Parameters *params) {
  // logarithm
  ...
  const complex<double> lmbd =
      aa * beta0 *
      log(params->mufs / S * pow(bb + nb / (1.0 + bb / nb * eta), 2));
  pdfEvolve(gE, qE, N, lmbd);
  CFact(gE, qE, N, aa, aa / (1.0 - lmbd));
  ...
  // Flux, symmetry factor, spin and color average
  return (RES * JtG(ll, ln, S, params->murs, params->set) - EXP) * dij / 24.0 /
         S;
}
    ...
    for (int i1 = 0; i1 < 5; i1++) {
      ...
      if (is_charge_conserved(params->in1, params->in2, params->out1,
        ...
        EXP += (qN[0][i0] * qN[1 - params->ic][i1] +
               ...
               (brn + vir * 4.0 / 3.0 * aa +
                (ll * (-ll * 4.0 / 3.0 + 4.0) -
                 2.0 * Pqq * (ll - log(S / params->mufs))) *
                    aa * brn);
        EXP += (qN[0][i0] + qN[1 - params->ic][i1] + qN[params->ic][i0] +
                qN[1][i1]) *
               gN * (log(S / params->mufs) - ll) * Pqg * aa * brn;
      }
    }
  }
  // Flux, symmetry factor, spin and color average
