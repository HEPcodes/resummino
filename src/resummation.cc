...
// NNLL for Drell-Yan like processes.
// Turn off to get ordinary NLL, instead of ordinary NNLL results.
#define NNLL

// Threshold resummation.
// G-exponent function for the resummation
complex<double> ThG(complex<double> LL, double S, double DIJ,
                    double Muf2, double Mur2, int set, Parameters *params) {
    ...
    const double A3 = 2.0 * 0.25 * cF * (pow2(cA) * (245.0/24.0 - 67.0/9.0 * zeta2 + 11.0/6.0 * zeta3
                                               + 11.0/5.0 * pow2(zeta2))
                                   + cF * dnf * (-55.0/24.0 + 2.0 * zeta3)
                                   + cA * dnf * (-209.0/108.0 + 10.0/9.0*zeta2 - 7.0/3.0*zeta3)
                                   - pow2(dnf)/27.0);

    // Only initial state dependent.
    // Needed for NNLL. 
    const double D2_i = 2.0 * cF * ( cA * (-101.0/27.0 + 11.0/3.0 * zeta2 + 7.0/2.0 * zeta3)
                               + dnf * (14.0/27.0 - 2.0/3.0 * zeta2));

    // Soft anomalous dimension for the associated gaugino-gluino production
    const double D1_ij = DIJ;

    // Second order soft anomalous dimenstion for the associated gaugino-gluino production 
    // This result is only valid for absolute threshold resummation (beta-threshold)!
    // const double D2_ij = cA * (-cA * (115.0/36.0 - 0.5 * zeta2 + 0.5 * zeta3) + 11.0/18.0 * dnf);
    ...
    // g2 = (g2_a + g2_b)
    g2 = A1 * b1 / (2.0 * M_PI * pow(b0,3))
      * (2.0 * lmbd + lnlmbd + 0.5 * pow2(lnlmbd))
    ...
    return exp(LL * g1 + g2 + h2 + alphaS * g3);
}

// Threshold resummation: (unintegrated) hadronic XS for ordinary NLL (NNLL) resummation
complex<double> Thadronic_xs(complex<double> NM, double S,
    ...
    // Collinear remainder part of the hard matching coefficient for Drell-Yan like processes.
        ...
        for (int i1 = 0; i1 < 5; i1++) {
            ...
            if (is_charge_conserved(params->in1, params->in2, params->out1, params->out2)) {
                ...
                if (is_gaugino_gluino(params->out1, params->out2)) {
                    ...
                    double P_T = -3.0/4.0 * ((2.0 * cF - cA) * log(mufs/S) + cA * log(mufs/(mgls - T)));
                    double P_U = -3.0/4.0 * ((2.0 * cF - cA) * log(mufs/S) + cA * log(mufs/(mgls - U)));
                    
                    double K_T = pow2(M_PI)/2.0 * cF - GAMMA_Q - K_Q
                    ...
                    double K_U = pow2(M_PI)/2.0 * cF - GAMMA_Q - K_Q
                    ...
                    // actuall H1_coll, but Jonathan shifted terms from integrated Dipole to coll. remainder.
                ...
                if(is_gaugino_gluino(params->out1, params->out2)) {
                    EXP += (qN[0][i0] * qN[1 - params->ic][i1] + qN[params->ic][i0] * qN[1][i1]) *
                       ( alphaS / (2.0 * M_PI) * H1 + 
                         H0 * (1.0 +  alphaS / (M_PI) * lnNb * (-DIJ + 2.0*cF * (log(mufs / params->murs) + lnNb
                                                                                 - log(S / params->murs)  )) ));
                } else {
                    EXP += (qN[0][i0] * qN[1 - params->ic][i1] + qN[params->ic][i0] * qN[1][i1]) *
                        (alphaS / (2.0 * M_PI) * H1 + H0 + 
                         H0 * alphaS / (M_PI) * lnNb * 2.0*cF * (log(mufs / params->murs) + lnNb
                                                                 - log(S / params->murs) ));
    ...
    // Hard matching function times exponential; expansion subctracted; color and spin average; flux factor.
    return (RES * ThG(lnNb, S, DIJ, params->mufs, params->murs, params->set, params) - EXP) * dij / 72.0 / S;
}

// Same as ThG, but with the improvement added (subleading non-diagonal splitting functions "readded")
// see Eq. 3.51f of Jonathan's PhD thesis.
complex<double> ThG2(const complex<double> LL,
                     double MIS, double MURS, const int set, Parameters *params) {
    ...
    // Exactly Eq. 3.52
    const complex<double> g2 = (2.0 * lmbd + lnlmbd)
...
complex<double> Thadronic_xs2(complex<double> N, double S, double T, Parameters *params) {
        ...
        for (int i1 = 0; i1 < 5; i1++) {
            ...
            if (is_charge_conserved(params->in1, params->in2, params->out1, params->out2)) {
                 if (is_slepton_slepton(params->out1, params->out2)) {
                    ...
                    // "small" vir is Eq. 3.50 (confusing, cause it includes parts of the collinear remainder!).
                    // Jon has shifted subtracted the 10 and some pi squared terms arising in DipI and coll.rem.
                    // vir = pow(M_PI, 2) / 3.0 * brn + Virt_gauginos(S, T, params) + DipI_gauginos(S, T, params);
                    
                    // Using a new version of the hard matching coeff, since I reshifted the (10-pi^2) term
                    // from the integrated dipole to the collinear remainder, where it belongs to.
                    // my vir + (10-pi^2) leads to the old vir.
                    vir = Virt_gauginos(S, T, params) + DipI_gauginos(S, T, params);
                    vir += 2.0 *(cF * pow2(M_PI)/2.0 - GAMMA_Q  - K_Q) / cF * brn;
                } else if (is_lepton_lepton(params->out1, params->out2)) {
                    brn = born_leptons(S, T, params);
                    //vir = pow(M_PI, 2) / 3.0 * brn + Virt_leptons(S, T, params) + DipI_leptons(S, T, params);
                ...
                EXP += (qN[0][i0] * qN[1 - params->ic][i1] + qN[params->ic][i0] * qN[1][i1]) 
                    * (brn + vir * 4.0 / 3.0 * alphaS / (2.0 * M_PI) +
                        (2.0 * ll * (-2.0 * ll * 4.0 / 3.0 + 4.0) 
                         - 2.0 * Pqq * (2.0 * ll - log(S / params->mufs))) * alphaS / (2.0 * M_PI) * brn);
    ...
    // Flux, symmetry factor, spin and color average. fc = 3; average 1/36; 1/2s for flux; and expansion subtracted
...
complex<double> PtG(const complex<double> LL,
    ...
    const complex<double> g2 = (lmbd / (1.0 - lmbd) + lnlmbd)
...
complex<double> JtG(const complex<double> LL,
    ...
    // LL + NLL Sudakov
    const complex<double> g1 = A1 * (1.0 + lnlmbd / lmbd);
    const complex<double> g2 = (lmbd / (1.0 - lmbd) * (1.0 - lmbdN) + lnlmbd)
                               * (A1 * log(MIS / MURS) - A2) + A1 * beta1 * (.5 * pow(lnlmbd, 2) +
                                       (1.0 - lmbdN) / (1.0 - lmbd) * (lmbd + lnlmbd)) + B1 * lnlmbd;
