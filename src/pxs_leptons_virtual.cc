    ...
    for (int i0 = 0; channels[qs][i0] > -1; i0++) {
        for (int i1 = 0; channels[qs][i1] > -1; i1++) {
            struct Coupling Cw[4] = {
                params->vll[ channels[qs][i0] ][ii - 20][jj - 20],
            ...
            ff->SetPropagator(params->mv[ channels[qs][i0] ],
                              params->mv[ channels[qs][i1] ],
                              params->Gv[ channels[qs][i0] ],
                              params->Gv[ channels[qs][i1] ]);
            ff->SetWCoupling(Cw);
...
double DipI_leptons(const double S, const double T, Parameters *params) {
    ...
    if (IEPS == 2) {
        ...
        return (10.0 - pow(M_PI,2) + (3.0 + lnmusq) * lnmusq) * born_leptons(S, T, params)
            + (3 + 2.0 * lnmusq) * born_leptons_eps1(S, T, params)
            + 2.0 * born_leptons_eps2(S, T, params);
    } else {
        return 0.0;
