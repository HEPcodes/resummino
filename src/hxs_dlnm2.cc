...
#define PT2CUT                                                                 \
  0.0 // pt2 cut for the integration (non-zero for joint resummation)

// two-particle phase space used for LO and virtual corrections
double dPS2_dlnM2(double &xa, double &xb, double &t, double *x,
                  Parameters *params) {
...
double dPS2C_dlnM2(double &djacdelta, double &djacplus, double &xa, double &xb,
                   double &xc, double &tc, double *x, Parameters *params) {
  const double sh = params->sh;
...
double dPS3_dlnM2(double &xa, double &xb, double &M2, double &pt2, double &Tp,
  ...
  // Phase Space factor.
  djac *= kln(M2, m1s, m2s) * sin(Tp) / sqrt(pow(s - M2, 2) - 4.0 * s * pt2) /
          pow(4.0 * M_PI, 4);
...
double dPS3_ONSHELL23_dlnM2(double &xa, double &xb, double &s2, double &t1,
  ...
  double s1max = m1s + m2s + 1.0 / (2.0 * s2) * (s - s2 - m1s) * (s2 + m2s) +
                 1.0 / (2.0 * s2) * (kln(s, s2, m1s) * kln(s2, m2s, 0));
  if (s1min > M2 || s1max < M2 || Rkln(s, s2, m1s) < 0.0 ||
      Rkln(s2, m2s, 0) < 0.0) {
    djac = 0.0;
    return djac;
  }
...
double dPS3_ONSHELL13_dlnM2(double &xa, double &xb, double &s2, double &t1,
  ...
  if (s1min > M2 || s1max < M2 || Rkln(s, s2, m2s) < 0.0 ||
      Rkln(s2, m1s, 0) < 0.0) {
    djac = 0.0;
...
double IB_dlnM2(double *x, size_t dim, void *jj) {
  Parameters *params =
      (Parameters *)jj; // creating an instance of the parameter class
  ...
  if (is_gaugino_gluino(params->out1,
                        params->out2)) { // LO with strong coupling.
    double g3s = std::norm(params->gqq[0][0].R);
    return 4.0 * M_PI * aS(params->murs, params->set) / g3s * 1.0 /
           (24.0 * s * 3.0) * sig * djac;
    ...
    for (int i1 = 0; i1 < 5; i1++) {
      ...
      if (is_charge_conserved(params->in1, params->in2, params->out1,
        ...
        if (is_gaugino_gluino(params->out1, params->out2)) {
          ...
          // s-> sc and sa1c -> sa1d.
          sig +=
              (qa[0][i0] * qd[1 - params->ic][i1] +
               qd[params->ic][i0] * qa[1][i1]) *
              (
          ...
          sig += (qa[0][i0] * qb[1 - params->ic][i1] +
                  qb[params->ic][i0] * qa[1][i1] +
                  qb[0][i0] * qa[1 - params->ic][i1] +
                  qa[params->ic][i0] * qb[1][i1]) *
                 (P_bold_aap_b_j(PARTON_QUARK, PARTON_QUARK, PARTON_ANTIQUARK,
          ...
          sig += (qa[0][i0] * qd[1 - params->ic][i1] +
                  qd[params->ic][i0] * qa[1][i1] +
                  qd[0][i0] * qa[1 - params->ic][i1] +
                  qa[params->ic][i0] * qd[1][i1]) *
                 (-P_bold_aap_b_j(PARTON_QUARK, PARTON_QUARK, PARTON_ANTIQUARK,
                                  PARTON_NONE, FUNCTION_PLUS, xc, z, 0.0, sc,
                                  params->mufs, 1.0, INITIAL) *
                  djacplus * bornc);

          // delta distribution part.
          sig += 2.0 * (qa[0][i0] * qd[1 - params->ic][i1] +
                        qd[params->ic][i0] * qa[1][i1]) *
                 (P_bold_aap_b_j(PARTON_QUARK, PARTON_QUARK, PARTON_ANTIQUARK,
                                 PARTON_NONE, FUNCTION_DELTA, xc, z, 0.0, sc,
                                 params->mufs, 1.0, INITIAL) *
                  djacdelta * bornc);
...
double IG_dlnM2j(double *x, size_t dim, void *jj) {
  ...
  // Flux, symmetry factor, spin and color average
  return dij * pow(M_PI, 2) / 4.5 / s * sig * djac *
         aS(params->murs, params->set) / (2 * M_PI);
}
...
double IQ23_dlnM2(double *x, size_t dim, void *jj) {
  ...
  if (is_gaugino_gluino(params->out1, params->out2)) {
    ...
    // params);
    djac23 =
        dPS3_ONSHELL23_dlnM2(xa, xb, s2_23, t1_23, s1_23, phi_23, x, params);
  }
...
double IR_dlnM2(double *x, size_t dim, void *prm) {
  ...
  const complex<double> nm =
      (s / sh / .09) + .09 - params->a1min - ephi * log(x[0]);
  djac *= ephi / x[0];
  ...
  // Collinear improved version for Drell-Yan like processes
  return M_1_PI * imag(djac * pow(s / sh, -nm + 1.0) *
                       Thadronic_xs2(nm, s, t, params)); // NLL impr.
}
...
void hadronic_xs_dlnM2(double &res, double &err, double &chi2, int Flag,
  ...
  case 0:
    ...
    break;
  case 1:
    Integration(&IV_dlnM2, 2, 800, params->precision, 1e-12, res, err, params,
                0.2);
    break;
  case 2:
    Integration(&IC_dlnM2, 3, 15000, params->precision, 1e-12, res, err,
                params);
    break;

  case 3:
    Integration(&IG_dlnM2, 5, 20000, params->precision, 1e-12, res, err,
                params);
    break;
  ...
  case 4:
    ...
    if (is_gaugino_gluino(params->out1, params->out2)) {
      ...
      Integration(&IQ_dlnM2, 5, 50000, params->precision, 1e-12, res, err,
                  params, 0.25, 5, 5);
      Integration(&IQ23_dlnM2, 5, 200000, params->precision, 1e-12, res23, err,
                  params, 0.25, 5, 5);
      int calls = 50000;
      for (int i = 0; i < 12; i++) {
        if (i == 8 || i == 11) {
      ...
      Integration(&IQ13_dlnM2, 5, calls, params->precision, 1e-12, res13, err,
                  params, 0.25, 5, 5);
      res = res + res13 + res23;
      err = sqrt(pow(err, 2) + pow(err13, 2) + pow(err23, 2));
    } else {
      Integration(&IQ_dlnM2, 5, 40000, params->precision, 1e-12, res, err,
                  params, 0.25, 5, 5);
    }
    break;
  case 5: // threshold resummation (improved for DY; ordinary for
          // gaugino-gluino)
    Integration(&IR_dlnM2, 2, 20000, params->precision, 1e-12, res, err, params,
                0.2, 5, 5);
    break;
  case 6: // ordinary threshold resummation (including nnll logs)
    Integration(&IR_nll_unimproved_dlnM2, 2, 20000, params->precision, 1e-12, res,
                err, params, 0.2, 5, 5);
    break;
  case 7: // ordinary threshold resummation (including nnll logs)
    Integration(&IR_nnll_unimproved_dlnM2, 2, 20000, params->precision, 1e-12, res,
                err, params, 0.2, 5, 5);
    break;
  default:
    break;
