    ...
    if (qt >= 0) {
        for (int i0 = 6 * qt; i0 < 6 * qt + 6; i0++) {
            for (int i1 = 6 * qt; i1 < 6 * qt + 6; i1++) {
                ...
                ff->SetWCoupling(Cw);
#ifdef BO1
                virt += ff->MVtbo1t(0.0, 0.0, ff->m1s, ff->m2s, 2.0 * ff->papb, ff->m1s - 2.0 * ff->pap1, params->mqs[bb],  0.0, params->mqs[aa], params->mSQs[i0], IEPS);                                
#endif
#ifdef TR1
                virt += ff->MVttr1t(0.0, ff->m1s - 2.* ff->pap1, ff->m1s, params->mqs[aa], 0.0, params->mSQs[i0],  IEPS);                
#endif
#ifdef TR2
                virt += ff->MVttr2t(0.0, ff->m1s - 2.* ff->pap1, ff->m2s, params->mqs[bb], 0.0, params->mSQs[i0],  IEPS);
#endif
#ifdef BU3
    ...
    if (qu <= 1) {
        for (int i0 = 6 * qu; i0 < 6 * qu + 6; i0++) {
            for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                struct Coupling Cw[4] = {
                    params->CHqSQ[ii][bb][i0],
                    params->CHSQq[jj][i0][aa],
                ...
                ff->SetWCoupling(Cw);
#ifdef BO1
                virt += ff->MVubo1u(0.0, 0.0, ff->m2s, ff->m1s, 2.0 * ff->papb, ff->m2s - 2.0 * ff->pap2, params->mqs[bb],  0.0, params->mqs[aa], params->mSQs[i0], IEPS);
#endif
#ifdef TR1
                virt += ff->MVutr1u(0.0, ff->m2s - 2.0*ff->pap2, ff->m2s, params->mqs[aa], 0.0, params->mSQs[i0],  IEPS);
#endif
#ifdef TR2
                virt += ff->MVutr2u(0.0, ff->m2s - 2.* ff->pap2, ff->m1s, params->mqs[bb], 0.0, params->mSQs[i0],  IEPS);
#endif
#ifdef BU3
    ...
    if (qt >= 0) {
        for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
            for (int i1 = 6 * qt; i1 < 6 * qt + 6; i1++) {
                ...
                ff->SetPropagator(params->mv[i0], params->mSQ[i1], params->Gv[i0], params->mSQ[i1] * 1.0e-2);
                ff->SetWCoupling(Cw1);
#ifdef TR1S
                ...
                ff->SetPropagator(params->mSQ[i1], params->mv[i0] , params->mSQ[i1] * 1.0e-2, params->Gv[i0]);
                ff->SetWCoupling(Cw2);
#ifdef BO1
                virt += ff->MVtbo1s(0.0, 0.0, ff->m1s, ff->m2s, 2.0 * ff->papb, ff->m1s - 2.0 * ff->pap1, params->mqs[bb],  0.0, params->mqs[aa], params->mSQs[i1], IEPS);
#endif
#ifdef TR1
                virt += ff->MVttr1s(0.0, ff->m1s - 2.0 * ff->pap1, ff->m1s, params->mqs[aa], 0.0, params->mSQs[i1],  IEPS);
#endif
#ifdef TR2
                virt += ff->MVttr2s(0.0, ff->m1s - 2.* ff->pap1, ff->m2s, params->mqs[bb], 0.0, params->mSQs[i1],  IEPS);
#endif
#ifdef BU3
    ...
    if (qu <= 1) {
        for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
            for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                ...
                ff->SetWCoupling(Cw2);
#ifdef BO1
                virt += ff->MVubo1s(0.0, 0.0, ff->m2s, ff->m1s, 2.0 * ff->papb, ff->m2s - 2.0 * ff->pap2, params->mqs[bb],  0.0, params->mqs[aa], params->mSQs[i1], IEPS);
#endif
#ifdef TR1
                virt += ff->MVutr1s(0.0, ff->m2s - 2.0*ff->pap2,ff->m2s, params->mqs[aa], 0.0, params->mSQs[i1],  IEPS);
#endif
#ifdef TR2
                virt += ff->MVutr2s(0.0, ff->m2s - 2.* ff->pap2, ff->m1s, params->mqs[bb], 0.0, params->mSQs[i1],  IEPS );
#endif
#ifdef BU3
    ...
    if (qt >= 0 && qu <= 1) {
        for (int i0 = 6 * qt; i0 < 6 * qt + 6; i0++) {
            for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                ...
                ff->SetPropagator(params->mSQ[i0], params->mSQ[i1] , params->mSQ[i0] * 1.0e-2, params->mSQ[i1] * 1.0e-2);
                ff->SetWCoupling(Cw1);
#ifdef BO1
                virt += ff->MVtbo1u(0.0, 0.0, ff->m1s, ff->m2s, 2.0 * ff->papb, ff->m1s - 2.0 * ff->pap1, params->mqs[bb],  0.0, params->mqs[aa], params->mSQs[i0], IEPS);
#endif
#ifdef TR1
                virt += ff->MVttr1u(0.0, ff->m1s - 2.0*ff->pap1, ff->m1s, params->mqs[aa], 0.0, params->mSQs[i0],  IEPS);
#endif
#ifdef TR2
                virt += ff->MVttr2u(0.0, ff->m1s - 2.* ff->pap1, ff->m2s, params->mqs[bb], 0.0, params->mSQs[i0],  IEPS );
#endif
#ifdef BU3
                ...
                ff->SetPropagator(params->mSQ[i1], params->mSQ[i0], params->mSQ[i1] * 1.0e-2, params->mSQ[i0] * 1.0e-2);
                ff->SetWCoupling(Cw2);
#ifdef BO1
                virt += ff->MVubo1t(0.0, 0.0, ff->m2s, ff->m1s, 2.0 * ff->papb, ff->m2s - 2.0 * ff->pap2, params->mqs[bb],  0.0, params->mqs[aa], params->mSQs[i1], IEPS);
#endif
#ifdef TR1
                virt += ff->MVutr1t(0.0, ff->m2s - 2.0*ff->pap2, ff->m2s, params->mqs[aa], 0.0, params->mSQs[i1],  IEPS);
#endif
#ifdef TR2
                virt += ff->MVutr2t(0.0, ff->m2s - 2.* ff->pap2, ff->m1s, params->mqs[bb], 0.0, params->mSQs[i1],  IEPS);
#endif
#ifdef BU3
    ...
    for (int j0 = (bb / 3) * 6; j0 < (bb / 3 + 1) * 6; j0++) {
        for (int j1 = (bb / 3) * 3; j1 < (bb / 3 + 1) * 3; j1++) {
            ...
            for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
                for (int i1 = 2 * qs; i1 < qs + 2; i1++) {
                    struct Coupling Cw[4] = {
                        params->vCHCH[i0][ii][jj], params->vqq[i0][j1][aa],
            ...
            if (qt >= 0) {
                for (int i0 = 6 * qt; i0 < 6 * qt + 6; i0++) {
                    for (int i1 = 6 * qt; i1 < 6 * qt + 6; i1++) {
                        struct Coupling Cw[4] = {
                            params->CHqSQ[jj][j1][i0], params->CHSQq[ii][i0][aa],
                            params->CHqSQ[jj][bb][i1], params->CHSQq[ii][i1][aa]
                        };
                        if (is_coupling_null(Cw, 4) == 0) {
                            ff->SetPropagator(params->mSQ[i0], params->mSQ[i1] , params->mSQ[i0] * 1.0e-2, params->mSQ[i1] * 1.0e-2);
            ...
            if (qu <= 1) {
                for (int i0 = 6 * qu; i0 < 6 * qu + 6; i0++) {
                    for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                        struct Coupling Cw[4] = {
                            params->CHqSQ[ii][j1][i0], params->CHSQq[jj][i0][aa],
                            params->CHqSQ[ii][bb][i1], params->CHSQq[jj][i1][aa]
                        };
                        if (is_coupling_null(Cw, 4) == 0) {
                            ff->SetPropagator(params->mSQ[i0], params->mSQ[i1] , params->mSQ[i0] * 1.0e-2, params->mSQ[i1] * 1.0e-2);
            ...
            if (qt >= 0) {
                for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
                    for (int i1 = 6 * qt; i1 < 6 * qt + 6; i1++) {
                        ...
                        struct Coupling Cw2[4] = {
                            params->CHqSQ[jj][j1][i1], params->CHSQq[ii][i1][aa],
                            params->vCHCH[i0][ii][jj], params->vqq[i0][bb][aa]
                        };
            ...
            if (qu <= 1) {
                for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
                    for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                        ...
                        struct Coupling Cw2[4] = {
                            params->CHqSQ[ii][j1][i1], params->CHSQq[jj][i1][aa],
                            params->vCHCH[i0][ii][jj], params->vqq[i0][bb][aa]
                        };
                        if (is_coupling_null(Cw1, 4) == 0) {
                            ff->SetPropagator(params->mv[i0], params->mSQ[i1], params->Gv[i0], params->mSQ[i1] * 1.0e-2);
        ...
        for (int j0 = 6 * qt; j0 < 6 * qt + 6; j0++) {
            for (int j1 = 6 * qt; j1 < 6 * qt + 6; j1++) {
                for (int j2 = 3 * qt; j2 < 3 * qt + 3; j2++) {
                    ...
                    for (int i1 = 6 * qt; i1 < 6 * qt + 6; i1++) {
                        struct Coupling Cw[4] = {
                            params->CHqSQ[jj][bb][j0], params->CHSQq[ii][j1][aa],
                            params->CHqSQ[jj][bb][i1], params->CHSQq[ii][i1][aa]
                        };
                        if (is_coupling_null(Cw, 4) == 1) {
                            continue;
                        }
                        ff->SetPropagator(params->mSQ[i1], params->mSQ[i1], params->mSQ[i1] * 1.0e-2, params->mSQ[i1] * 1.0e-2);
                        ff->SetWCoupling(Cw);
#ifdef BU4
                        virt += ff->MVtbu4t(ff->m1s - 2.0* ff->pap1, params->mqs[j2], params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS);

#endif
                        if (j1 == j0) {
#ifdef BU4C
                            virt += ff->MVtct4t(params->mSQs[j1], params->mqs[j2], params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS);
#endif
                        }                         
                    }
#endif
                    ...
                    for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
                        ...
                        if (is_coupling_null(Cw, 4) == 0) {
                            ...
                            ff->SetWCoupling(Cw);
#ifdef BU4
                            virt += ff->MVtbu4s(ff->m1s - 2.0* ff->pap1, params->mqs[j2], params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS);
#endif
                            if (j1 == j0) {
#ifdef BU4C
                                virt += ff->MVtct4s(params->mSQs[j1], params->mqs[j2], params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS);
#endif
                            }
                        }
                    }
                    ...
                    if (qu <= 1) {
                        for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                            ...
                            if (is_coupling_null(Cw, 4) == 0) {
                                ...
                                ff->SetWCoupling(Cw);
#ifdef BU4
                                virt += ff->MVtbu4u(ff->m1s - 2.0* ff->pap1, params->mqs[j2], params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS);
                                
#endif
                                if (j1 == j0) {
#ifdef BU4C
                                    virt += ff->MVtct4u(params->mSQs[j1], params->mqs[j2], params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS);
#endif
                                }
                            }
                        }
        ...
        for (int j0 = 6 * qu; j0 < 6 * qu + 6; j0++) {
            for (int j1 = 6 * qu; j1 < 6 * qu + 6; j1++) {
                for (int j2 = 3 * qu; j2 < 3 * qu + 3; j2++) {
                    ...
                    for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                        struct Coupling Cw[4] = {
                            params->CHqSQ[ii][bb][j0], params->CHSQq[jj][j1][aa],
                            params->CHqSQ[ii][bb][i1], params->CHSQq[jj][i1][aa]
                        };
                        if (is_coupling_null(Cw, 4) == 0) {
                            ff->SetPropagator(params->mSQ[i1], params->mSQ[i1], params->mSQ[i1] * 1.0e-2, params->mSQ[i1] * 1.0e-2);
                            ff->SetWCoupling(Cw);
#ifdef BU4
                            virt += ff->MVubu4u(ff->m2s - 2.0* ff->pap2, params->mqs[j2], params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS);
                            
#endif
                            if (j1 == j0) {
#ifdef BU4C
                                virt += ff->MVuct4u(params->mSQs[j1], params->mqs[j2], params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS);
#endif
                            }
                        }
                    }
                    ...
                    for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
                        ...
                        if (is_coupling_null(Cw, 4) == 0) {
                            ...
                            ff->SetWCoupling(Cw);
#ifdef BU4
                            virt += ff->MVubu4s(ff->m2s - 2.0* ff->pap2, params->mqs[j2], params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS);
#endif
                            
                            if (j1 == j0) {
#ifdef BU4C
                                virt += ff->MVuct4s(params->mSQs[j1], params->mqs[j2], params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS);
#endif
                            }
                        }
                    }
                    ...
                    if (qt >= 0) {
                        for (int i0 = 6 * qt; i0 < 6 * qt + 6; i0++) {
                            ...
                            if (is_coupling_null(Cw, 4) == 0) {
                                ff->SetPropagator(params->mSQ[i0], params->mSQ[i0], params->mSQ[i0] * 1.0e-2, params->mSQ[i0] * 1.0e-2);
                                ff->SetWCoupling(Cw);
#ifdef BU4
                                virt += ff->MVubu4t(ff->m2s - 2.0* ff->pap2, params->mqs[j2], params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS);
#endif
                                if (j1 == j0) {
#ifdef BU4C
                                    virt += ff->MVuct4t(params->mSQs[j1], params->mqs[j2], params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS);
#endif
                                }
                            }
                        }
...
// infrared safe diagrams for internal squark self energy with a squark inside the loop
...
double Vbu5(const double S, const double T, Parameters *params) {
    ...
    if (qt >= 0) {
        for (int j0 = 6 * qt; j0 < 6 * qt + 6; j0++) {
            for (int j1 = 6 * qt; j1 < 6 * qt + 6; j1++) {
                if (j0 == j1) {
                    continue;    // Counterterm: dm2 = diag(SelfEnergy); The counterterm exactly cancels the diagram
                }
    ...
    for (int j0 = (bb / 3) * 6; j0 < (bb / 3 + 1) * 6; j0++) {
        for (int j1 = (aa / 3) * 6; j1 < (aa / 3 + 1) * 6; j1++) {
            ...
            for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
                for (int i1 = 2 * qs; i1 < qs + 2; i1++) {
                    struct Coupling Cw[4] = {
                        params->vCHCH[i0][ii][jj], params->vSQSQ[i0][j0][j1],
                        params->vCHCH[i1][ii][jj], params->vqq[i1][bb][aa]
                    };
                    if (is_coupling_null(Cw, 4) == 0) {
                        ff->SetPropagator(params->mv[i0], params->mv[i1] , params->Gv[i0], params->Gv[i1]);
                        ff->SetWCoupling(Cw);
#ifdef TR2S
                        virt += ff->MVstr2s(0.0, 0.0, 2.0 * ff->papb, params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS );
#endif
                    }
                }
            }
...
#ifdef TR2S
                            ff->SetWCoupling(Cw);
                            virt += ff->MVstr2t(0.0, 0.0, 2.0 * ff->papb, params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS );
#endif
                        }
                    }
                }
                ...
                for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
                    for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                        ...
                        if (is_coupling_null(Cw, 4) == 0) {
                            ...
                            ff->SetWCoupling(Cw);
#ifdef TR2S
                            virt += ff->MVstr2u(0.0, 0.0, 2.0 * ff->papb, params->mGLs, params->mSQs[j0], params->mSQs[j1],  IEPS );
#endif
                        }
                    }
                }
        ...
        for (int j1 = (aa / 3) * 6; j1 < (aa / 3 + 1) * 6; j1++) {
            ...
            if (qt >= 0) {
                for (int i0 = 4 * qs; i0 < 2 * qs + 4; i0++) {
                    for (int i1 = 6 * qt; i1 < 6 * qt + 6; i1++) {
                        struct Coupling Cw[4] = {
                            params->hCHCH[i0][ii][jj], params->hSQSQ[i0][j0][j1],
                            params->CHqSQ[jj][bb][i1], params->CHSQq[ii][i1][aa]
                        };
                        if (is_coupling_null(Cw, 4) == 0) {
                            ff->SetPropagator(params->mh[i0], params->mSQ[i1], params->mh[i0] * 1.0e-2, params->mSQ[i1] * 1.0e-2);
                            ff->SetWCoupling(Cw);
#ifdef TR3
                            virt += ff->MVstr3t(0.0, 0.0, 2.0 * ff->papb,  params->mGLs,params->mSQs[j0] ,  params->mSQs[j1],  IEPS);
#endif
                        }
                    }
                }
                ...
                for (int i0 = 4 * qs; i0 < 2 * qs + 4; i0++) {
                    for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                        ...
                        if (is_coupling_null(Cw, 4) == 0) {
                            ...
                            ff->SetWCoupling(Cw);
#ifdef TR3
                            virt += ff->MVstr3u(0.0, 0.0, 2.0 * ff->papb,  params->mGLs,params->mSQs[j0] ,  params->mSQs[j1],  IEPS);
#endif
                        }
                    }
                }
        ...
        for (int j0 = (aa / 3) * 6; j0 < (aa / 3 + 1) * 6; j0++) {
            for (int j1 = qt * 3; j1 < qt * 3 + 3; j1++) {
                for (int j2 = qt * 6; j2 < qt * 6 + 6; j2++) {
                    ...
                    for (int i1 = 6 * qt; i1 < 6 * qt + 6; i1++) {
                        struct Coupling Cw[4] = {
                            params->CHqSQ[jj][bb][j2], params->CHqSQ[ii][j1][j0],
                            params->CHqSQ[jj][bb][i1], params->CHSQq[ii][i1][aa]
                        };
                        if (is_coupling_null(Cw, 4) == 0) {
                            ff->SetPropagator(params->mSQ[j2], params->mSQ[i1], params->mSQ[j2] * 1.0e-2, params->mSQ[i1] * 1.0e-2);
                            ff->SetWCoupling(Cw);
#ifdef TR2A
                            virt += ff->MVttr3t(0.0, ff->m1s - 2.0 * ff->pap1, ff->m1s,  params->mGLs,params->mSQs[j0] ,  0.0,  IEPS);
#endif
                        }
                    }
#endif
                    ...
                    for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
                        ...
                        if (is_coupling_null(Cw, 4) == 0) {
                            ff->SetPropagator(params->mSQ[j2], params->mv[i0], params->mSQ[j2] * 1.0e-2, params->Gv[i0]);
                            ff->SetWCoupling(Cw);
#ifdef TR2A
                            virt += ff->MVttr3s(0.0, ff->m1s - 2.0 * ff->pap1, ff->m1s,  params->mGLs,params->mSQs[j0] ,  params->mqs[j1],  IEPS);
#endif
                        }
                    }
#endif
                    ...
                    if (qu <= 1) {
                        for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                            ...
                            if (is_coupling_null(Cw, 4) == 0) {
                                ...
                                ff->SetWCoupling(Cw);
#ifdef TR2A
                                virt += ff->MVttr3u(0.0, ff->m1s - 2.0 * ff->pap1, ff->m1s,  params->mGLs,params->mSQs[j0] ,  params->mqs[j1],  IEPS);
#endif
                            }
                        }
                    }
            ...
            for (int j1 = qt * 3; j1 < qt * 3 + 3; j1++) {
                for (int j2 = qt * 6; j2 < qt * 6 + 6; j2++) {
                    ...
                    if (params->out1 >= 20) {
                        ...
                        for (int i1 = 6 * qt; i1 < 6 * qt + 6; i1++) {
                            struct Coupling Cw[4] = {
                                params->CHSQq[jj][j0][j1], params->CHSQq[ii][j2][aa],
                                params->CHqSQ[jj][bb][i1], params->CHSQq[ii][i1][aa]
                            };
                            if (is_coupling_null(Cw, 4) == 0) {
                                ff->SetPropagator(params->mSQ[j2], params->mSQ[i1], params->mSQ[j2] * 1.0e-2, params->mSQ[i1] * 1.0e-2);
                                ff->SetWCoupling(Cw);
#ifdef TR2B
                                virt += ff->MVttr4t(0.0, ff->m1s - 2.0 * ff->pap1, ff->m2s,  params->mGLs,params->mSQs[j0] ,  params->mqs[j1],  IEPS);
                        ...
                        for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
                            ...
                            if (is_coupling_null(Cw, 4) == 0) {
                                ...
                                ff->SetWCoupling(Cw);
#ifdef TR2B
                                virt += ff->MVttr4s(0.0, ff->m1s - 2.0 * ff->pap1, ff->m2s,  params->mGLs,params->mSQs[j0] ,  params->mqs[j1],  IEPS);
#endif
                            }
                        }
#endif
                        ...
                        if (qu <= 1) {
                            for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                                ...
                                if (is_coupling_null(Cw, 4) == 0) {
                                    ...
                                    ff->SetWCoupling(Cw);
#ifdef TR2B
                                    virt += ff->MVttr4u(0.0, ff->m1s - 2.0 * ff->pap1, ff->m2s,  params->mGLs,params->mSQs[j0] ,  params->mqs[j1],  IEPS);
#endif
                                }
                            }
                        }
        ...
        for (int j0 = (aa / 3) * 6; j0 < (aa / 3 + 1) * 6; j0++) {
            for (int j1 = qu * 3; j1 < qu * 3 + 3; j1++) {
                for (int j2 = qu * 6; j2 < qu * 6 + 6; j2++) {
                    ...
                    for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                        struct Coupling Cw[4] = {
                            params->CHqSQ[ii][bb][j2], params->CHqSQ[jj][j1][j0],
                            params->CHqSQ[ii][bb][i1], params->CHSQq[jj][i1][aa]
                        };
                        if (is_coupling_null(Cw, 4) == 0) {
                            ff->SetPropagator(params->mSQ[j2], params->mSQ[i1], params->mSQ[j2] * 1.0e-2, params->mSQ[i1] * 1.0e-2);
                            ff->SetWCoupling(Cw);
#ifdef TR2A
                            virt += ff->MVutr3u(0.0, ff->m2s - 2.0 * ff->pap2, ff->m2s,  params->mGLs,params->mSQs[j0] ,  params->mqs[j1],  IEPS);
#endif
                        }
                    }
#endif
                    ...
                    for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
                        ...
                        if (is_coupling_null(Cw, 4) == 0) {
                            ...
                            ff->SetWCoupling(Cw);
#ifdef TR2A
                            virt += ff->MVutr3s(0.0, ff->m2s - 2.0 * ff->pap2, ff->m2s,  params->mGLs,params->mSQs[j0] ,  params->mqs[j1],  IEPS);
#endif
                        }
                    }
#endif
                    ...
                    if (qt >= 0) {
                        for (int i1 = 6 * qt; i1 < 6 * qt + 6; i1++) {
                            ...
                            if (is_coupling_null(Cw, 4) == 0) {
                                ...
                                ff->SetWCoupling(Cw);
#ifdef TR2A
                                virt += ff->MVutr3t(0.0, ff->m2s - 2.0 * ff->pap2, ff->m2s,  params->mGLs,params->mSQs[j0] ,  params->mqs[j1],  IEPS);
#endif
                            }
                        }
                    }
        ...
        for (int j0 = (bb / 3) * 6; j0 < (bb / 3 + 1) * 6; j0++) {
            for (int j1 = qu * 3; j1 < qu * 3 + 3; j1++) {
                for (int j2 = qu * 6; j2 < qu * 6 + 6; j2++) {
                    ...
                    for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                        struct Coupling Cw[4] = {
                            params->CHSQq[ii][j0][j1], params->CHSQq[jj][j2][aa],
                            params->CHqSQ[ii][bb][i1], params->CHSQq[jj][i1][aa]
                        };
                        if (is_coupling_null(Cw, 4) == 0) {
                            ff->SetPropagator(params->mSQ[j2], params->mSQ[i1], params->mSQ[j2] * 1.0e-2, params->mSQ[i1] * 1.0e-2);
                            ff->SetWCoupling(Cw);
#ifdef TR2B
                            virt += ff->MVutr4u(0.0, ff->m2s - 2.0 * ff->pap2, ff->m1s,  params->mGLs,params->mSQs[j0] ,  params->mqs[j1],  IEPS);
#endif
                        }
                    }
#endif
                    ...
                    for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
                        ...
                        if (is_coupling_null(Cw, 4) == 0) {
                            ...
                            ff->SetWCoupling(Cw);
#ifdef TR2B
                            virt += ff->MVutr4s(0.0, ff->m2s - 2.0 * ff->pap2, ff->m1s,  params->mGLs,params->mSQs[j0] ,  params->mqs[j1],  IEPS);
#endif
                        }
                    }
#endif
                    ...
                    if (qt >= 0) {
                        for (int i1 = 6 * qt; i1 < 6 * qt + 6; i1++) {
                            ...
                            if (is_coupling_null(Cw, 4) == 0) {
                                ...
                                ff->SetWCoupling(Cw);
#ifdef TR2B
                                virt += ff->MVutr4t(0.0, ff->m2s - 2.0 * ff->pap2, ff->m1s,  params->mGLs,params->mSQs[j0] ,  params->mqs[j1],  IEPS);
#endif
                            }
                        }
                    }
                ...
                for (int j2 = qt * 3; j2 < 3 * qt + 3; j2++) {
                    ...
                    for (int i1 = 6 * qt; i1 < 6 * qt + 6; i1++) {
                        ...
                        if (is_coupling_null(Cw, 4) == 0) {
                            ...
                            ff->SetWCoupling(Cw);
#ifdef BO2
                            virt += ff->MVtbo2t(0.0, 0.0, ff->m1s, ff->m2s, 2.0 * ff->papb, ff->m1s - 2.0 * ff->pap1, params->mSQs[j0],  params->mGLs, params->mSQs[j1], params->mqs[j2], IEPS);
#endif
                        }
                    }
#endif
                    ...
                    for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
                        ...
                        if (is_coupling_null(Cw, 4) == 0) {
                            ...
                            ff->SetWCoupling(Cw);
#ifdef BO2
                            virt += ff->MVtbo2s(0.0, 0.0, ff->m1s, ff->m2s, 2.0 * ff->papb, ff->m1s - 2.0 * ff->pap1, params->mSQs[j0],  params->mGLs, params->mSQs[j1], params->mqs[j2], IEPS);
#endif
                        }
                    }
#endif
                    ...
                    if (qu <= 1) {
                        for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                            ...
                            if (is_coupling_null(Cw, 4) == 0) {
                                ...
                                ff->SetWCoupling(Cw);
#ifdef BO2
                                virt += ff->MVtbo2u(0.0, 0.0, ff->m1s, ff->m2s, 2.0 * ff->papb, ff->m1s - 2.0 * ff->pap1, params->mSQs[j0],  params->mGLs, params->mSQs[j1], params->mqs[j2], IEPS);
#endif
                            }
                        }
                    }
                ...
                for (int j2 = qu * 3; j2 < 3 * qu + 3; j2++) {
                    ...
                    for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                        ...
                        if (is_coupling_null(Cw, 4) == 0) {
                            ...
                            ff->SetWCoupling(Cw);
#ifdef BO2
                            virt += ff->MVubo2u(0.0, 0.0, ff->m2s, ff->m1s, 2.0 * ff->papb, ff->m2s - 2.0 * ff->pap2, params->mSQs[j0],  params->mGLs, params->mSQs[j1], params->mqs[j2], IEPS);
#endif
                        }
                    }
#endif
                    ...
                    for (int i0 = 2 * qs; i0 < qs + 2; i0++) {
                        ...
                        if (is_coupling_null(Cw, 4) == 0) {
                            ...
                            ff->SetWCoupling(Cw);
#ifdef BO2
                            virt += ff->MVubo2s(0.0, 0.0, ff->m2s, ff->m1s, 2.0 * ff->papb, ff->m2s - 2.0 * ff->pap2, params->mSQs[j0],  params->mGLs, params->mSQs[j1], params->mqs[j2], IEPS);
#endif
                        }
                    }
#endif
                    ...
                    if (qt >= 0) {
                        for (int i0 = 6 * qt; i0 < 6 * qt + 6; i0++) {
                            ...
                            if (is_coupling_null(Cw, 4) == 0) {
                                ...
                                ff->SetWCoupling(Cw);
#ifdef BO2
                                virt += ff->MVubo2t(0.0, 0.0, ff->m2s, ff->m1s, 2.0 * ff->papb, ff->m2s - 2.0 * ff->pap2, params->mSQs[j0],  params->mGLs, params->mSQs[j1], params->mqs[j2], IEPS);
#endif
                            }
                        }
                    }
...
double Ctt3(const double S, const double T, Parameters *params) {
    ...
    if (qt >= 0 && IEPS == 1) {
        for (int i0 = 6 * qt; i0 < 6 * qt + 6; i0++) {
            for (int i1 = 6 * qt; i1 < 6 * qt + 6; i1++) {
                struct Coupling Cw[4] = {
                    params->CHqSQ[jj][bb][i0], params->dCHSQq[ii][i0][aa],
                    params->CHqSQ[jj][bb][i1], params->CHSQq[ii][i1][aa]
                };
                if (is_coupling_null(Cw, 4) == 1) {
                    continue;
...
double Ctt4(const double S, const double T, Parameters *params) {
    ...
    if (qt >= 0 && IEPS == 1) {
        for (int i0 = 6 * qt; i0 < 6 * qt + 6; i0++) {
            for (int i1 = 6 * qt; i1 < 6 * qt + 6; i1++) {
                struct Coupling Cw[4] = {
                    params->dCHqSQ[jj][bb][i0], params->CHSQq[ii][i0][aa],
                    params->CHqSQ[jj][bb][i1], params->CHSQq[ii][i1][aa]
                };
                if (is_coupling_null(Cw, 4) == 1) {
                    continue;
...
double Ctu3(const double S, const double T, Parameters *params) {
    ...
    if (qu <= 1 && IEPS == 1) {
        for (int i0 = 6 * qu; i0 < 6 * qu + 6; i0++) {
            for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                struct Coupling Cw[4] = {
                    params->CHqSQ[ii][bb][i0], params->dCHSQq[jj][i0][aa],
                    params->CHqSQ[ii][bb][i1], params->CHSQq[jj][i1][aa]
                };
                if (is_coupling_null(Cw, 4) == 1) {
                    continue;
...
double Ctu4(const double S, const double T, Parameters *params) {
    ...
    if (qu <= 1 && IEPS == 1) {
        for (int i0 = 6 * qu; i0 < 6 * qu + 6; i0++) {
            for (int i1 = 6 * qu; i1 < 6 * qu + 6; i1++) {
                struct Coupling Cw[4] = {
                    params->dCHqSQ[ii][bb][i0], params->CHSQq[jj][i0][aa],
                    params->CHqSQ[ii][bb][i1], params->CHSQq[jj][i1][aa]
                };
                if (is_coupling_null(Cw, 4) == 1) {
                    continue;
...
double Virt_gauginos(const double S, const double T, Parameters *params) {
    ...
    // vertex counterterms (generates only finite pieces for inteference channels with S)
...
double DipI_gauginos(const double S, const double T, Parameters *params) {
    ...
    if (IEPS == 2) {
        ...
        return ((3.0 + 2.0 * lnmusq) * born_gauginos(S, T, params) + 2.0 * born_gauginos_eps1(S, T, params));
    } else if (IEPS == 0) {
        //old: 10 - pi^2 missing: Jonathan shifted those pieces to the coll. remainder.
        ...
        return (10.0 - pow(M_PI,2) + ( 3.0 + lnmusq) * lnmusq) * born_gauginos(S, T, params)
                              + (3 + 2.0 * lnmusq) * born_gauginos_eps1(S, T, params) + 2.0 * born_gauginos_eps2(S, T, params);
    } else {
        return 0.0;
    }
#endif
