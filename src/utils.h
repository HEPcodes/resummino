...
// particle/antiparticle pair. And the one of the squarks(not antisquarks) + constant (1/3)
...
static inline bool is_charge_conserved(int in1, int in2, int out1, int out2) {
  ...
  if ( is_squark_gaugino(out1,out2) ) {
    if (out1 > 30) {
      return quark_charge_plus_constant(in1) == particle_charge(out1) - particle_charge(out2);
    } else {
      return -quark_charge_plus_constant(in2) == particle_charge(out1) - particle_charge(out2);
    }
  } else {
    return quark_charge_plus_constant(in1) - quark_charge_plus_constant(in2)
...
#define JET_KINEMATICS                                                  \
    if (ii < 10 && jj < 10) {                                           \
        ...
        ff->SetKinematic(params->mCH[ii], params->mSQ[jj - 31], S, M2, PT2, TH, PH, YS); \
    }
