...
static inline double kln(double x, double y, double z) {
  return sqrt(pow(x, 2) + pow(y, 2) + pow(z, 2) - 2.0 * x * y - 2.0 * y * z -
              2.0 * z * x);
}

// kaellen function.
static inline double Rkln(double x, double y, double z) {
  return pow(x, 2) + pow(y, 2) + pow(z, 2) - 2.0 * x * y - 2.0 * y * z -
         2.0 * z * x;
}

// universal G-function.
static inline double G(double x, double y, double z, double u, double v,
                       double w) {
  return pow(v, 2) * w + v * pow(w, 2) + u * v * x + u * w * y + pow(x, 2) * y +
         x * pow(y, 2) + pow(u, 2) * z + w * x * z + w * y * z -
         u * (v + w + x + y) * z + u * pow(z, 2) - x * y * (u + v + w + z) -
         v * w * (u + x + y + z);
}
...
static inline int quark_charge_plus_constant(int quark) {
  return quark < 3 ? 0 : 1;
}

// first row are the down type (s)quarks, second row are the up type ones
static const int squark_type[][7] = {{0, 3, 1, 4, 2, 5, -1},
                                     {6, 9, 7, 10, 8, 11, -1}};
...
static inline int propagator_charge(int in1, int in2, int out1, int out2,
                                    ChannelName channel) {
...
static inline bool is_gaugino_gaugino(int out1, int out2) {
  return (out1 < 10 && out2 < 10);
}
...
static inline bool is_charge_conserved(int in1, int in2, int out1, int out2) {
  ...
  if (is_squark_gaugino(out1, out2)) {
    if (out1 > 30) {
      return quark_charge_plus_constant(in1) ==
             particle_charge(out1) - particle_charge(out2);
    } else {
      return -quark_charge_plus_constant(in2) ==
             particle_charge(out1) - particle_charge(out2);
    }
  } else {
    return quark_charge_plus_constant(in1) - quark_charge_plus_constant(in2) ==
...
#define BORN_KINEMATICS                                                        \
  if (ii < 10 && jj < 10) {                                                    \
    ...
    ff->SetKinematic(params->mCH[ii], params->mSQ[jj - 31], S, T);             \
  }

#define JET_KINEMATICS                                                         \
  if (ii < 10 && jj < 10) {                                                    \
    ff->SetKinematic(params->mCH[ii], params->mCH[jj], S, M2, PT2, TH, PH,     \
                     YS);                                                      \
  } else if (ii >= 10 && ii < 20) {                                            \
    ff->SetKinematic(params->mSL[ii - 10], params->mSL[jj - 10], S, M2, PT2,   \
                     TH, PH, YS);                                              \
  } else if (ii >= 20 && ii < 30) {                                            \
    ff->SetKinematic(params->ml[ii - 20], params->ml[jj - 20], S, M2, PT2, TH, \
                     PH, YS);                                                  \
  } else if (ii == 30 && jj < 10) {                                            \
    ...
    ff->SetKinematic(params->mSQ[ii - 31], params->mCH[jj], S, M2, PT2, TH,    \
                     PH, YS);                                                  \
  } else if (jj >= 31 && ii < 10) {                                            \
    ff->SetKinematic(params->mCH[ii], params->mSQ[jj - 31], S, M2, PT2, TH,    \
                     PH, YS);                                                  \
  }
