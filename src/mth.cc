...
complex<double> Gamma(const complex<double> x)
{
    ...
    // Works with Re(xx) > 0
    const complex<double> xx = (real(x) > 0. ? x : -x);
    ...
    complex<double> gamma = exp((xx + .5) * log(xx + 5.5) - xx - 5.5)
                            * (q0 + q1 * xx + q2 * pow(xx, 2) + q3 * pow(xx, 3)
                               + q4 * pow(xx, 4)
                               + q5 * pow(xx, 5) + q6 * pow(xx, 6))
                            / xx / (xx + 1.0) / (xx + 2.0) / (xx + 3.0) / (xx + 4.0)
                            / (xx + 5.0) / (xx + 6.0);
...
complex<double> Psi(complex<double> x)
{
    ...
    result += log(x) - .5 / x - pow(x, -2) / 2520.
              *(210. + pow(x, -2) * (-21. + pow(x, -2) * 10.));

    return result;
}

complex<double> Beta(const complex<double> x,
