...
void FI::SetKinematic_HelicityFrame(const double mi, const double mj,
  ...
  if (flag == 1) {
    ...
    detM = -((s - s2) * (m2s * s - s1 * s2)) +
           (s * (-2 * m2s - s + s1) + (s + s1) * s2) * t1 +
           m1s * (m2s * s - 2 * s * s2 + s1 * s2 + s * t1 - s1 * t1);
    t2 = 1.0 / (Rkln(s, s2, m1s)) *
         (-detM +
          2.0 * sqrt(G(s, t1, s2, 0, 0, m1s) * G(s1, s2, s, m2s, m1s, 0)) *
              cosphi);
  ...
  if (flag == 2) {
    detM = -((s - s2) * (m1s * s - s1 * s2)) +
           (s * (-2 * m1s - s + s1) + (s + s1) * s2) * t1 +
           m2s * (m1s * s - 2 * s * s2 + s1 * s2 + s * t1 - s1 * t1);
    t2 = 1.0 / (Rkln(s, s2, m2s)) *
         (-detM +
          2.0 * sqrt(G(s, t1, s2, 0, 0, m2s) * G(s1, s2, s, m1s, m2s, 0)) *
              cosphi);
...
void FI::SetKinematicRES23(const double mSQs, double &factor) {
  ...
  pap1 = kln(Qs, mSQs, m1s) / kln(Qs, 2.0 * p2p3 + m2s, m1s) *
             (pap1 - p1Q / Qs * paQ) +
         (Qs + m1s - mSQs) / (2 * Qs) * paQ;

  // set pbp1 to on shell kinematics
  pbp1 = kln(Qs, mSQs, m1s) / kln(Qs, 2.0 * p2p3 + m2s, m1s) *
             (pbp1 - p1Q / Qs * pbQ) +
...
void FI::SetKinematicRES13(const double mSQs, double &factor) {
  ...
  pap2 = kln(Qs, mSQs, m2s) / kln(Qs, 2.0 * p1p3 + m1s, m2s) *
             (pap2 - p2Q / Qs * paQ) +
         (Qs + m2s - mSQs) / (2 * Qs) * paQ;

  pbp2 = kln(Qs, mSQs, m2s) / kln(Qs, 2.0 * p1p3 + m1s, m2s) *
             (pbp2 - p2Q / Qs * pbQ) +
...
void FI::SetKinematic(const double mi, const double mj, const double s,
  ...
  if (ys == 0) {
    pap3 = 0.25 * (s - invariant_mass2 +
                   sqrt(pow2(s - invariant_mass2) - 4.0 * s * pt2)); // 3
    //      pbp3 = 0.25 * (s - invariant_mass2 - sqrt(pow2(s - invariant_mass2)
    //      - 4.0 * s * pt2));
  } else if (ys == 1) {
    pap3 = 0.25 * (s - invariant_mass2 -
                   sqrt(pow(s - invariant_mass2, 2) - 4.0 * s * pt2)); // 3
  ...
  cA = 1.0 -
       2.0 * s * invariant_mass2 / (invariant_mass2 + 2.0 * pap3) /
           (invariant_mass2 + 2.0 * (-0.5 * (m1s + m2s) - p1p2 - pap3 + papb));
  sA = sqrt(1.0 - cA * cA);

  pbp1 = .25 * (invariant_mass2 + 2.0 * pap3) / invariant_mass2 *
         (invariant_mass2 + m1s - m2s -
          kM12 * (cA * cos(th) + sA * sin(th) * cos(ph))); // 5
...
void FI::SetDipKinematicAB(double &x) {
  ...
  const double pap1_temp =
      ...
      ((pap3 - papb + pbp3) *
       (pap3 - 2 * papb + 2 * pbp3 + pap3 * x - 2 * papb * x));
...
void FI::SetDipKinematicBA(double &x) {
  ...
  const double pap1_temp =
      ((pap3 - papb + pbp3) * ((pap3 - papb) * (p1p3 - pbp1) + pap1 * pbp3) +
       ((pap3 - papb) * (p1p3 * papb - 2 * pap1 * papb - pap3 * pbp1) +
        (pap1 * (pap3 - 2 * papb) + (-pap3 + papb) * pbp1) * pbp3 +
        pap1 * pow(pbp3, 2)) *
           x +
       papb * ((pap3 + papb) * pbp1 + pap1 * (2 * papb - pbp3) +
               p1p3 * (-2 * papb + pbp3)) *
           pow(x, 2)) /
      ((pap3 - papb + pbp3) *
       (2 * pap3 - 2 * papb + pbp3 - 2 * papb * x + pbp3 * x));

  const double pap2_temp =
      ((pap3 - papb + pbp3) * ((pap3 - papb) * (p2p3 - pbp2) + pap2 * pbp3) +
       ((pap3 - papb) * (p2p3 * papb - 2 * pap2 * papb - pap3 * pbp2) +
        (pap2 * (pap3 - 2 * papb) + (-pap3 + papb) * pbp2) * pbp3 +
        pap2 * pow(pbp3, 2)) *
           x +
       papb * ((pap3 + papb) * pbp2 + pap2 * (2 * papb - pbp3) +
               p2p3 * (-2 * papb + pbp3)) *
           pow(x, 2)) /
      ((pap3 - papb + pbp3) *
       (2 * pap3 - 2 * papb + pbp3 - 2 * papb * x + pbp3 * x));
  ...
  const double pbp1_temp =
      (x * (2 * pow(pap3, 2) * pbp1 +
            pap3 * (-4 * papb * pbp1 + p1p3 * pbp3 + 2 * pbp1 * pbp3) +
            pap1 * (2 * pap3 * (papb - pbp3) +
                    (2 * papb - pbp3) * (2 * pbp3 + papb * (-1 + x))) +
            (2 * papb - pbp3) *
                (papb * pbp1 - p1p3 * pbp3 + papb * (-p1p3 + pbp1) * x))) /
      ((pap3 - papb + pbp3) *
       (2 * pap3 - 2 * papb + pbp3 - 2 * papb * x + pbp3 * x));

  const double pbp2_temp =
      (x * (2 * pow(pap3, 2) * pbp2 +
            pap3 * (-4 * papb * pbp2 + p2p3 * pbp3 + 2 * pbp2 * pbp3) +
            pap2 * (2 * pap3 * (papb - pbp3) +
                    (2 * papb - pbp3) * (2 * pbp3 + papb * (-1 + x))) +
            (2 * papb - pbp3) *
                (papb * pbp2 - p2p3 * pbp3 + papb * (-p2p3 + pbp2) * x))) /
      ((pap3 - papb + pbp3) *
       (2 * pap3 - 2 * papb + pbp3 - 2 * papb * x + pbp3 * x));
...
void FI::SetDipKinematic1A(double &x, double &zi, double &zj, double &zplus,
                           double &zminus, double mi, double mj, double mij) {

  x = 1 -
      (pow(mi, 2) - pow(mij, 2) + pow(mj, 2) + 2 * p1p3) / (2. * (pap1 + pap3));
  zi = pap3 / (pap3 + pap1);
  zj = pap1 / (pap3 + pap1);
  ...
  zplus = (1 - x + pow2(muij) + pow2(mui) - pow2(muj) +
           sqrt(pow2(1 - x + pow2(muij) - pow2(mui)) -
                4.0 * pow2(mui) * pow2(muj))) /
          (2.0 * (1 - x + pow2(muij)));
...
void FI::SetDipKinematic1B(double &x, double &zi, double &zj, double &zplus,
                           double &zminus, double mi, double mj, double mij) {

  x = 1 -
      (pow(mi, 2) - pow(mij, 2) + pow(mj, 2) + 2 * p1p3) / (2. * (pbp1 + pbp3));
  zi = pbp3 / (pbp3 + pbp1);
  zj = pbp1 / (pbp3 + pbp1);
...
void FI::SetDipKinematicA(double &x, double &fact) {
  ...
  pap1 = (pap3 * m1s + pap3 * p1p2 - 2.0 * papb * p1p3 + 2.0 * papb * pap1 +
          x * pap3 * m1s + x * pap3 * p1p3 + x * pap3 * p1p2 +
          2.0 * x * papb * pap1) /
         L;
  pap2 = (pap3 * m2s + pap3 * p1p2 - 2.0 * papb * p2p3 + 2.0 * papb * pap2 +
          x * pap3 * m2s + x * pap3 * p2p3 + x * pap3 * p1p2 +
          2.0 * x * papb * pap2) /
         L;
  pbp1 = (2.0 * papb * m1s + 2.0 * papb * p1p2 + 2.0 * papb * pbp1 -
          2.0 * x * pap3 * m1s - x * pap3 * p1p3 - 2.0 * x * pap3 * p1p2 -
          2.0 * x * papb * m1s - 2.0 * x * papb * p1p3 - 2.0 * x * papb * p1p2 +
          2.0 * x * papb * pbp1) /
         L;
  pbp2 = (2.0 * papb * m2s + 2.0 * papb * p1p2 + 2.0 * papb * pbp2 -
          2.0 * x * pap3 * m2s - x * pap3 * p2p3 - 2.0 * x * pap3 * p1p2 -
          2.0 * x * papb * m2s - 2.0 * x * papb * p2p3 - 2.0 * x * papb * p1p2 +
          2.0 * x * papb * pbp2) /
         L;
}
...
void FI::SetDipKinematicB(double &x, double &fact) {
  ...
  pbp1 = (pbp3 * m1s + pbp3 * p1p2 - 2.0 * papb * p1p3 + 2.0 * papb * pbp1 +
          x * pbp3 * m1s + x * pbp3 * p1p3 + x * pbp3 * p1p2 +
          2.0 * x * papb * pbp1) /
         L;
  pbp2 = (pbp3 * m2s + pbp3 * p1p2 - 2.0 * papb * p2p3 + 2.0 * papb * pbp2 +
          x * pbp3 * m2s + x * pbp3 * p2p3 + x * pbp3 * p1p2 +
          2.0 * x * papb * pbp2) /
         L;
  pap1 = (2.0 * papb * m1s + 2.0 * papb * p1p2 + 2.0 * papb * pap1 -
          2.0 * x * pbp3 * m1s - x * pbp3 * p1p3 - 2.0 * x * pbp3 * p1p2 -
          2.0 * x * papb * m1s - 2.0 * x * papb * p1p3 - 2.0 * x * papb * p1p2 +
          2.0 * x * papb * pap1) /
         L;
  pap2 = (2.0 * papb * m2s + 2.0 * papb * p1p2 + 2.0 * papb * pap2 -
          2.0 * x * pbp3 * m2s - x * pbp3 * p2p3 - 2.0 * x * pbp3 * p1p2 -
          2.0 * x * papb * m2s - 2.0 * x * papb * p2p3 - 2.0 * x * papb * p1p2 +
          2.0 * x * papb * pap2) /
         L;
}
...
void FI::SetBreitWigner(double ps, double ms, double DecayWidth) {
  BreitWigner =
      ((ms * pow2(DecayWidth))) / (pow2(ps - ms) + ms * pow2(DecayWidth));
}
