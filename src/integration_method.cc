...
void Integration(double (*IF)(double *x, size_t dim, void *jj), int ndim,
  ...
  gsl_monte_vegas_integrate(&I, xmin, xmax, dnum, calls * calls_warmupfactor, r,
                            s, &res, &err);
  ...
  for (int counter = 1; counter <= ((Parameters *)userdata)->max_iters > 0;
       counter++) {
    // if result is zero (e.g. LO pt) relative precision does not make any sense
    if (abs(res) < 1.0e-15 ||
        (prec < epsrel && fabs(gsl_monte_vegas_chisq(s) - 1.0) < 0.5) ||
        prec < epsrel / 2.5) {
      break;
    }
    s->iterations = 1;
