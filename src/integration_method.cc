...
void Integration(double (*IF)(double *x, size_t dim, void *jj),
                 int ndim, int maxeval, double epsrel,  double epsabs,
                 double &res, double &err,  void *userdata,
                 double calls_warmupfactor = 0.1, 
    ...
    gsl_monte_vegas_integrate(&I, xmin, xmax, dnum, calls * calls_warmupfactor, r, s,
                              &res, &err);

    // Integrates
   // Calling VEGAS with stage = 1 retains the grid from the previous run but discards the weighted average,
   // so that one can “tune” the grid using a relatively small number of points and then do a large run with stage = 1 
    s->stage = 1;
    ...
    for (int counter = 1; counter <= ((Parameters*)userdata)->max_iters > 0; counter++ ) {
        ...
        gsl_monte_vegas_integrate(&I, xmin, xmax, dnum, calls, r, s,
                                  &res, &err);
